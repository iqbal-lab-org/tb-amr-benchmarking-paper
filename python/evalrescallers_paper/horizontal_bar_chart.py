...
def make_plot(stats_dict, tools, drugs, outfile, first_line=False, ten_k=False, how_to_scale='not at all', susc_gap=None, susc_xticks=None, res_xticks=None, plot_gap_size=30):
    ...
    svg_lines = []
    svg_lines.append(svg.svg_text(0.5 * (x_susc_bar_left + x_centre - centre_space), y, 'Susceptible samples', 20))
    svg_lines.append(svg.svg_text(0.5 * (x_centre + centre_space + x_res_bar_right), y, 'Resistant samples', 20))
    y = 45
    ...
    svg_lines.append(svg.svg_line(x_susc_bar_left, y, x_centre - centre_space, y, 'black', 1))
    svg_lines.append(svg.svg_line(x_centre + centre_space, y, x_res_bar_right, y, 'black', 1))
    y_tick = y
    ...
    for drug in drugs:
        ...
        for tool in tools:
            ...
            svg_lines.append(svg.svg_rectangle(x_centre + centre_space, y, x_fail, y_bottom, common_data.other_colours['UNK_R'], 'black'))
            svg_lines.append(svg.svg_rectangle(x_fail, y, x_fn, y_bottom, common_data.other_colours['FN'], 'black'))
            svg_lines.append(svg.svg_rectangle(x_fn, y, x_right, y_bottom, common_data.tool_colours[tool], 'black'))
            ...
            svg_lines.append(svg.svg_rectangle(x_fail, y, x_centre - centre_space, y_bottom, common_data.other_colours['UNK_S'], 'black'))
            svg_lines.append(svg.svg_rectangle(x_fp, y, x_fail, y_bottom, common_data.other_colours['FN'], 'black'))
            svg_lines.append(svg.svg_rectangle(x_left, y, x_fp, y_bottom, common_data.tool_colours[tool], 'black'))
    ...
    for x, label in zip(res_ticks, res_ticks_labels):
        ...
        svg_lines.append(svg.svg_line(x_pos, y_tick-4, x_pos, y_tick, 'black', 1))
        svg_lines.append(svg.svg_text(x_pos, y_tick-7, label, 15))
        grid_lines.append(svg.svg_line(x_pos, y_tick, x_pos, y-drug_spacer, 'lightgray', 1))
    ...
    <linearGradient id="gradwhiteright" x1="0%" y1="0%" x2="100%" y2="0%">
      ...
      <stop offset="100%" style="stop-color:rgb(255,255,255);stop-opacity:-1" />
        ...
        svg_lines.append(svg.svg_rectangle(x_left, y_tick+1, x_middle, y+1, 'url(#gradwhiteleft)', 'white', border_width=0))
        svg_lines.append(svg.svg_rectangle(x_middle, y_tick+1, x_right, y+1, 'url(#gradwhiteright)', 'white', border_width=0))
        svg_lines.append(svg.svg_line(x_left+3, y_tick, x_right, y_tick, 'white', 1))
        svg_lines.append(svg.svg_line(x_right-3, y_tick+3, x_right+3, y_tick-3, 'black', 1))
        svg_lines.append(svg.svg_line(x_left, y_tick+3, x_left+6, y_tick-3, 'black', 1))
